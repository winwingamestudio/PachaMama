...
public class SingleJoystickPlayerController : MonoBehaviour
{
    ...
    public float moveSpeed = 1.0f; // Character movement speed.
    public int rotationSpeed = 8; // How quick the character rotate to target location.
    Vector3 input01;
    Rigidbody rigidBody;
    public static bool playerXMovementEnable;
    public static bool playerYMovementEnable;
    ...
    void FixedUpdate()
    {
        ...
        if (input01 != Vector3.zero)
        {
            ...
            if(!playerXMovementEnable)
            xMovementInput01 = 0;
            if(!playerYMovementEnable)
            yMovementInput01 = 0;
